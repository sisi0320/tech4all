...
public class AuthenticationFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		/*
		 * //if the user is not logged in and doesnot try to access home or access these
		 * servlet: if (!isLoggedIn && !uri.endsWith(StringUtils.HOME) && !(isLogin ||
		 * isLoginServlet || isRegisterPage || isRegisterServlet)) {
		 * res.sendRedirect(req.getContextPath() + StringUtils.LOGIN_PAGE); }// Redirect
		 * logged-in users to the index page if trying to access login page again else
		 * if (isLoggedIn && !uri.endsWith(StringUtils.HOME_PAGE) && !(!isLogin ||
		 * isLogoutServlet)) { res.sendRedirect(req.getContextPath() +
		 * StringUtils.HOME_PAGE); }//Redirect logged in admin to admin page if trying
		 * to access home page else if(isLoggedIn && isAdmin &&
		 * uri.endsWith(StringUtils.HOME_PAGE)) { res.sendRedirect(req.getContextPath()+
		 * StringUtils.ADMIN_PAGE); }//Redirect logged in users to home page if trying
		 * to access admin dashboard else if(isLoggedIn &&
		 * uri.endsWith(StringUtils.ADMIN_PAGE) && !isAdmin){
		 * res.sendRedirect(req.getContextPath() + StringUtils.HOME_PAGE); }else {
		 * logger.info("Allowing request"); // Allow access to the requested resource if
		 * user is logged in or accessing unprotected resources chain.doFilter(request,
		 * response); }
		 */
